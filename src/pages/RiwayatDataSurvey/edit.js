...
export default function InputDataSurvey({navigation, route}) {
  const { editMode = false, surveyData = null } = route.params || {};
  ...
  // State untuk foto (tidak diload dari data lama, selalu kosong untuk diganti)
  ...
  useEffect(() => {
    if (editMode && surveyData) {
      setFormData(surveyData.formData);
    }
  }, [editMode, surveyData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editMode && surveyData) {
        ...
        // Cari dan update survey yang sedang diedit
        const updatedSurveys = surveysArray.map(survey => {
          if (survey.id === surveyData.id) {
            return {
              ...survey,
              formData: formData,
              photos: photos, // Update foto juga
              lastModified: new Date().toISOString(),
              dateModified: new Date().toLocaleDateString('id-ID')
            };
          }
          return survey;
        });

        await AsyncStorage.setItem('surveyData', JSON.stringify(updatedSurveys));
        ...
        const surveyData = {
          id: surveyId,
          formData: formData,
          photos: photos, // Simpan foto untuk mode tambah baru
        ...
        Alert.alert('Berhasil!', 'Data survey berhasil disimpan!', [
          {
            text: 'OK',
            onPress: () => navigation.goBack()
          }
  ...
  return (
    <View style={styles.container}>
      <MyHeader title={editMode ? "Edit Survey" : "Input Survey"}/>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={styles.submitButton} onPress={handleSubmit}>
          <Text style={styles.submitButtonText}>
            {editMode ? 'UPDATE DATA SURVEY' : 'SIMPAN DATA SURVEY'}
          </Text>
        </TouchableOpacity>
