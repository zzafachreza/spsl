...
export default function RiwayatDataSurvey({navigation}) {
  ...
  const loadSurveyData = async () => {
    try {
      ...
      if (existingSurveys) {
        const surveysArray = JSON.parse(existingSurveys);
        // Urutkan berdasarkan timestamp terbaru
        const sortedSurveys = surveysArray.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
        setSurveyList(sortedSurveys);
      } else {
        setSurveyList([]);
      }
    } catch (error) {
      console.error('Error loading survey data:', error);
      Alert.alert('Error', 'Gagal memuat data survey');
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    }
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              const filteredSurveys = surveyList.filter(survey => survey.id !== surveyId);
              await AsyncStorage.setItem('surveyData', JSON.stringify(filteredSurveys));
              setSurveyList(filteredSurveys);
              Alert.alert('Berhasil', 'Data survey berhasil dihapus');
            } catch (error) {
              console.error('Error deleting survey:', error);
              Alert.alert('Error', 'Gagal menghapus data survey');
            }
          }
        }
      ]
      ...
      <View key={survey.id} style={styles.surveyCard}>
        ...
        <View style={styles.cardHeader}>
          <View style={styles.headerLeft}>
            <Text style={styles.surveyNumber}>Survey #{index + 1}</Text>
            <Text style={styles.surveyDate}>{survey.dateCreated}</Text>
          </View>
        ...
        <View style={styles.cardBody}>
          <View style={styles.dataRow}>
            ...
            <Text style={styles.dataValue}>{formData.nama || '-'}</Text>
          </View>
          
          <View style={styles.dataRow}>
            <Text style={styles.dataLabel}>Jenis Kelamin</Text>
            <Text style={styles.dataValue}>{formData.jenisKelamin === 'L' ? 'Laki-laki' : formData.jenisKelamin === 'P' ? 'Perempuan' : '-'}</Text>
          </View>

          <View style={styles.dataRow}>
            <Text style={styles.dataLabel}>No. KTP</Text>
            <Text style={styles.dataValue}>{formData.nomorKTP || '-'}</Text>
          </View>

          <View style={styles.dataRow}>
            <Text style={styles.dataLabel}>No. KK</Text>
            <Text style={styles.dataValue}>{formData.nomorKK || '-'}</Text>
          </View>

          <View style={styles.dataRow}>
            <Text style={styles.dataLabel}>Alamat</Text>
            <Text style={styles.dataValue} numberOfLines={2}>{formData.alamat || '-'}</Text>
          </View>

          <View style={styles.dataRow}>
            <Text style={styles.dataLabel}>Umur</Text>
            <Text style={styles.dataValue}>{formData.umur ? `${formData.umur} tahun` : '-'}</Text>
          </View>
          ...
          <View style={styles.dataRow}>
            ...
            <Text style={styles.dataValue}>{formData.pekerjaanSektor || '-'}</Text>
          </View>
          ...
          <View style={styles.dataRow}>
            ...
            <Text style={styles.dataValue}>{formData.statusKepemilikanRumah || '-'}</Text>
          </View>

          {formData.latitude && formData.longitude && (
            <View style={styles.dataRow}>
              <Text style={styles.dataLabel}>Koordinat</Text>
              <Text style={styles.dataValue}>{formData.latitude}, {formData.longitude}</Text>
            </View>
          )}
        </View>
